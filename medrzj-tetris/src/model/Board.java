...
public class Board extends Observable {
    ...
    /**
     * THe current holding piece.
     */
    private MovableTetrisPiece myHoldPiece;
    ...
    /**
     * Holds the current piece for the player.
    ...
    public void holdPiece() {
        if (myHoldPiece == null) {
            ...
            myHoldPiece = myCurrentPiece;

            if (myCurrentPiece.getTetrisPiece() == TetrisPiece.O) {
                final Point newPosition = new Point(position.x() + 1, position.y());
                myCurrentPiece = nextMovablePiece(false).setPosition(newPosition);
            } else {
                myCurrentPiece = nextMovablePiece(false).setPosition(position);
                
                final TetrisPiece[] piece = new TetrisPiece[1];
                piece[0] = myHoldPiece.getTetrisPiece();
                setChanged();
                notifyObservers(piece);
            }
        } else {

            final MovableTetrisPiece temp = myCurrentPiece;
            final Point position = myCurrentPiece.getPosition();
            myCurrentPiece =  myHoldPiece.setPosition(position);
            
            // Sets new position if the piece is placed outside
            // of the board bounds. 
            final int xPoint = position.x() + myCurrentPiece.getTetrisPiece().getWidth();
            
            if (xPoint > DEFAULT_WIDTH) {
                final int x = xPoint - 10;
                final Point newPosition = new Point(position.x() - x, position.y());
                myCurrentPiece = myHoldPiece.setPosition(newPosition);
            } else if (temp.getTetrisPiece() == TetrisPiece.O) {
                final Point newPosition = new Point(position.x() + 1, position.y());
                myCurrentPiece =  myHoldPiece.setPosition(newPosition);
            } else if (myCurrentPiece.getTetrisPiece() == TetrisPiece.O) {
                final Point newPosition = new Point(position.x() - 1, position.y());
                myCurrentPiece =  myHoldPiece.setPosition(newPosition);
            }
